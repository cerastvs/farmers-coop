...
function ApplicantPendingScreen({
  ...
  initialStatus: ApplicationFeeStatus | null;
}) {
  const [status, setStatus] = useState<ApplicationFeeStatus | null>(
    initialStatus,
  );
  ...
  let icon: React.ReactNode = (
    <CreditCard className="text-amber-600 w-10 h-10" />
  );
  let iconBg = "bg-yellow-100";
  let title = "Application fee required";
  let body =
    "Your application was submitted. Pay the application fee to continue with your membership.";
  ...
  if (pending) {
    ...
    body =
      "Your payment proof is being verified by the President. You will be notified once it is approved.";
    ...
    body =
      "Your application fee has been verified. Your membership is now being activated.";
    actionLabel = "View payment status";
        ...
        <div className="rounded-[2.5rem] border border-[#dce8d2] bg-white p-10 text-center shadow-xl shadow-[#173a2b]/[.06]">
            ...
            <div className="mb-8 max-w-md mx-auto space-y-4 rounded-3xl border border-red-200 bg-red-50 p-6 text-left">
              ...
              {status?.application.reviewedBy && (
                <p className="text-xs text-red-700/80">
                  Decided by:{" "}
                  {status.application.reviewedBy.name ??
                    status.application.reviewedBy.username}{" "}
                  — President
                  {status.application.reviewedAt
                    ? ` · ${new Date(status.application.reviewedAt).toLocaleString(
                        "en-PH",
                        {
                          month: "short",
                          day: "2-digit",
          ...
          <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 mt-8">
            ...
            <p className="text-gray-700">
              ...
              <span className="font-bold text-[#51a808]">
                &quot;Edit Profile&quot;
              </span>{" "}
              in the menu above.
            </p>
